...
inputPlaying (EventKey c Down _ _) e@E {player = p}
  ...
  where setPrevs f = f {prev_dX = getdX f, prev_dY = getdY f}
        ...
        jumpX = ignoringJump (\n f -> let step = laneDiff * signum n in setdX n . setPrevs $ f {is_JumpingX = True, targetX = getX f + step})
        jumpY = ignoringJump (\n f -> let step = laneDiff * signum n in setdY n . setPrevs $ f {is_JumpingY = True, targetY = getY f + step})
inputPlaying _ e = e
